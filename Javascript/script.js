        ...
        reader.onload = async (e) => {
            try {
                ...
                jsonLimpio.Productos.forEach(producto => {
                    longitudesMaximas.IdProducto = Math.max(longitudesMaximas.IdProducto, producto.IdProducto.toString().length);
                    longitudesMaximas.Nombre = Math.max(longitudesMaximas.Nombre, producto.Nombre.length);
                    longitudesMaximas.Formulado = Math.max(longitudesMaximas.Formulado, producto.Formulado.length);
                    longitudesMaximas.Num_Registro = Math.max(longitudesMaximas.Num_Registro, producto.Num_Registro.length);
                    longitudesMaximas.Estado = Math.max(longitudesMaximas.Estado, producto.Estado.length);
                    longitudesMaximas.Titular = Math.max(longitudesMaximas.Titular, producto.Titular.length);
                    longitudesMaximas.Fabricante = Math.max(longitudesMaximas.Fabricante, producto.Fabricante.length);

                    producto.Usos.forEach(uso => {
                        if (uso.VolumenCaldo) {
                            longitudesMaximas.VolumenCaldo = Math.max(longitudesMaximas.VolumenCaldo, uso.VolumenCaldo.length);
                        }
                        if (uso.CondicionamientoEspecifico) {
                            longitudesMaximas.CondicionamientoEspecifico = Math.max(longitudesMaximas.CondicionamientoEspecifico, uso.CondicionamientoEspecifico.length);
                        }
                        if (uso.Agente) {
                            longitudesMaximas.Agente = Math.max(longitudesMaximas.Agente, uso.Agente.length);
                        }
                        if (uso.Cultivo) {
                            longitudesMaximas.Cultivo = Math.max(longitudesMaximas.Cultivo, uso.Cultivo.length);
                        }
                        if (uso.CodigoCultivo) {
                            longitudesMaximas.CodigoCultivo = Math.max(longitudesMaximas.CodigoCultivo, uso.CodigoCultivo.length);
                        }
                        if (uso.CodigoAgente) {
                            longitudesMaximas.CodigoAgente = Math.max(longitudesMaximas.CodigoAgente, uso.CodigoAgente.length);
                        }
                        if (uso.UnidadMedidaDosis) {
                            longitudesMaximas.UnidadMedidaDosis = Math.max(longitudesMaximas.UnidadMedidaDosis, uso.UnidadMedidaDosis.length);
                        }
                        if (uso.PlazoSeguridad) {
                            longitudesMaximas.PlazoSeguridad = Math.max(longitudesMaximas.PlazoSeguridad, uso.PlazoSeguridad.length);
                        }
                        if (uso.Aplicaciones) {
                            longitudesMaximas.Aplicaciones = Math.max(longitudesMaximas.Aplicaciones, uso.Aplicaciones.length);
                        }
                        if (uso.IntervaloAplicaciones) {
                            longitudesMaximas.IntervaloAplicaciones = Math.max(longitudesMaximas.IntervaloAplicaciones, uso.IntervaloAplicaciones.length);
                        }
                        if (uso.MetodoAplicacion) {
                            longitudesMaximas.MetodoAplicacion = Math.max(longitudesMaximas.MetodoAplicacion, uso.MetodoAplicacion.length);
                        }
                        if (uso.UnidadesVolumen) {
                            longitudesMaximas.UnidadesVolumen = Math.max(longitudesMaximas.UnidadesVolumen, uso.UnidadesVolumen.length);
                        }
                    });
                });
